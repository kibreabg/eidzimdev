...
{	
   $entid=$row['ID'];
   		$entname=trim($row['name']);
		$rs2 = mysql_query( "CALL GetNationalResultbyEntrypoint($entid,1,$mwaka,$mwezi, @numsamples)" );
...
{	
   $entid=$row['ID'];
   		$entname=trim($row['name']);
		$rs2 = mysql_query( "CALL GetNationalResultbyEntrypoint($entid,2,$mwaka,$mwezi, @numsamples)" );
$rs2 = mysql_query( "SELECT @numsamples as 'numsamples'" );
$d2=mysql_fetch_array($rs2);
$posi=$d2['numsamples'];
