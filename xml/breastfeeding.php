...
<categories>
<?php 
      $sql="select ID,name from feedings   ";
...
{	
      $feedid=$row['ID'];
   		$feedname=trim($row['name']);?><set value="<?php echo Getfeedingpositivitycount($feedid,1,$mwaka,$mwezi)?>"/><?php }?></dataset>
<dataset seriesName="Positive" showValues="0">
 <?php while($row=mysql_fetch_array($result3))
{	
      $feedid=$row['ID'];
   		$feedname=trim($row['name']);		   	?><set value="<?php echo Getfeedingpositivitycount($feedid,2,$mwaka,$mwezi)?>"/><?php }?></dataset>
