...
header("Content-Disposition: attachment;Filename=DateRange_Excel_Report.xls");
...
include("nationaldashboardfunctions.php");
	...
	//for th query
	$fromfilter=date("Y-m-d",strtotime($startdate));
	$tofilter=date("Y-m-d",strtotime($enddate));
	//for th display
	$fromfilterdisplay=date("d-M-Y",strtotime($fromfilter));
	$tofilterdisplay=date("d-M-Y",strtotime($tofilter));
	$partnername=GetPartnerName($partner);	
		
					if (($province==0) &&  ($facility==0) ) //all
					{
				
						$title= $fromfilterdisplay . " - " .$tofilterdisplay;
						$showfacility = "SELECT samples.ID,samples.patientid,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples WHERE facilitys.ID=samples.facility  AND samples.datetested between '$fromfilter'  AND  '$tofilter'  AND samples.repeatt=0 AND  samples.flag=1 ";
					...
					elseif (($province ==0) &&  ($facility !=0) )//facility
					{
						...
						$title= $facilityname . " ,".$fromfilterdisplay . " - " .$tofilterdisplay;
						
						$showfacility = "SELECT samples.ID,samples.patientid,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples WHERE facilitys.ID=samples.facility AND samples.facility='$facility' AND samples.datetested between '$fromfilter'  AND  '$tofilter' AND samples.repeatt=0 AND  samples.flag=1 ";
					...
					$objQuery = @mysql_query($showfacility) or die(mysql_error());
						$reportitle= strtoupper($partnername) ." TEST OUTCOME REPORT  BETWEEN ". strtoupper($title) ;
