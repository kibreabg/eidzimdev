...
header("Content-Disposition: attachment;Filename=Half_Year_Excel_Report.xls");
//readfile('Half_Year_Excel_Report.xls');
?>
<?php
include("nationaldashboardfunctions.php");
	...
	$quotaname= GetBiAnnualName($biannual);
	

	if ($biannual == 1 ) //january - March
		{
		$startmonth=1;
		$endmonth=6;
		}
		...
		$endmonth=12;
					...
					if (($province==0) &&  ($facility==0) ) //all
					{
						...
						$title= " BETWEEN ".$quotaname . " , ".$biannualyear;
						$showfacility = "SELECT samples.ID,samples.patientid,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples WHERE facilitys.ID=samples.facility  AND YEAR(samples.datetested)='$biannualyear' AND MONTH(samples.datetested) BETWEEN '$startmonth' AND '$endmonth' AND samples.repeatt=0 AND  samples.flag=1 ";
					...
					elseif (($province !=0) &&  ($facility==0) ) //provincial
					{
						...
						$title= $provname . " Province ,  BETWEEN ". $quotaname . " , ".$biannualyear;
						$showfacility = "SELECT samples.ID,samples.patientid,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples,districts WHERE facilitys.ID=samples.facility AND facilitys.district=districts.ID  AND districts.province='$province' AND YEAR(samples.datetested)='$biannualyear' AND MONTH(samples.datetested) BETWEEN '$startmonth' AND '$endmonth' AND samples.repeatt=0 AND  samples.flag=1 ";
			
					}
				/*	else if ($radio=="radio3") //district
					{
						$distname=GetDistrictName($district);
						$title= $distname . " District , BETWEEN ". $quotaname . " , ".$biannualyear;
						$showfacility = "SELECT samples.ID,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples WHERE facilitys.ID=samples.facility AND facilitys.district='$district' AND YEAR(samples.datetested)='$biannualyear' AND MONTH(samples.datetested) BETWEEN '$startmonth' AND '$endmonth' AND samples.repeatt=0 AND  samples.flag=1 ";
					...
					elseif (($province ==0) &&  ($facility !=0) )//facility
					{
						...
						$title= $facilityname . " ,  BETWEEN ".$quotaname . " , ".$biannualyear;
						
						$showfacility = "SELECT samples.ID,samples.patientid,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples WHERE facilitys.ID=samples.facility AND samples.facility='$facility' AND  YEAR(samples.datetested)='$biannualyear' AND MONTH(samples.datetested) BETWEEN '$startmonth' AND '$endmonth' AND samples.repeatt=0 AND  samples.flag=1 ";
		
					}
				/*	else if ($radio=="radio5")//lab
					{
						$labname= GetLab($lab);
						$title= $labname . " ,  BETWEEN ".$quotaname . " , ".$biannualyear;
						
						$showfacility = "SELECT samples.ID,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM samples WHERE samples.lab as 'labtestedin'='$lab' AND  YEAR(samples.datetested)='$biannualyear' AND MONTH(samples.datetested) BETWEEN '$startmonth' AND '$endmonth' AND samples.repeatt=0 AND  samples.flag=1 ";
