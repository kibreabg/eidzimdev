...
header("Content-Disposition: attachment;Filename=Monthly_Excel_Report.xls");
//readfile('Monthly_Excel_Report'.'.xls');
?>
<?php
include("nationaldashboardfunctions.php");
	...
	$monthname=strtoupper(GetMonthName($monthly));
					...
					elseif (($province ==0) &&  ($facility !=0) )//facility
					{
						...
						$title= $facilityname . " ,".$monthname . " - " .$monthyear;
						
						$showfacility = "SELECT samples.ID,samples.patientid,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples WHERE facilitys.ID=samples.facility AND samples.facility='$facility' AND  YEAR(samples.datetested)='$monthyear' AND MONTH(samples.datetested)= $monthly AND samples.repeatt=0 AND  samples.flag=1 ";
		
					}
					/*else if ($radio=="radio5")//lab
					{
						//lab of facility
						$labname= GetLab($lab);
						$title= $labname . " ,".$monthname . " - " .$monthyear;
						
						$showfacility = "SELECT samples.ID,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM samples WHERE  samples.lab as 'labtestedin'='$lab' AND  YEAR(samples.datetested)='$monthyear' AND MONTH(samples.datetested)= $monthly AND samples.repeatt=0 AND  samples.flag=1 ";
	...
	<tr >
	  ...
	  <th>Sex</th><th>Age</th><th>DOB</th><th> Baby Prophylaxis</th><th> Tested Before(1=Yes,2=No,3=Unknown)</th>
	  <th>Date of Collection </th><th> HIV Status of Mother</th><th>PMTCT Intervention</th><th>Breast Feeding </th><th> Is Mother On Art(1=Yes,2=No,3=Unknown) </th><th>Entry Point</th><th>Date of Receiving </th><th>Date of Testing</th><th>Date of Dispatch </th><th>Test Result</th></tr>
	...
	//get patient gender
		$pgender=$gender;
		//tested before
		//get patient prophylaxis 04/04/17
		//$prophylaxis =GetBabyProphylaxis($patientid);
		$bprophylaxis= GetProphylaxisName($prophylaxis);
				
		$ptestedbefore=$testedbefore;
		...
		//get mothers if on art or not
		$onart=GetmotheronART($mother);
		//get entry point
