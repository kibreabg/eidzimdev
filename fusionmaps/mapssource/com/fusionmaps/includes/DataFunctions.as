...
function loadData() {
	...
	if (isDataURLProvided()) {
		...
		if (_isSafeDataURL){
			...
			map.log ("dataURL reset", "A javascript/asfunction was invoked in dataURL, which can be potentially dangerous as it allows XSS attacks. Re-setting dataURL to Data.xml. ", Logger.LEVEL.ERROR);
		}
		...
		//Raise the necessary data event
		if (xmlData.status==0){
			if (_registerWithJS && ExternalInterface.available){
				ExternalInterface.call ("FC_DataLoaded", _DOMId);
			}
		}else {
			if (_registerWithJS && ExternalInterface.available){
				ExternalInterface.call ("FC_DataXMLInvalid", _DOMId);
			}
		}
		//Goto FDataLoadDone
