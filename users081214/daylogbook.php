...
$displaydatefilter=date("d-M-Y",strtotime($datefilter));
...
$qury = "SELECT *
        FROM samples,facilitys WHERE samples.datereceived = '$datefilter' AND samples.Flag = 1 AND samples.facility=facilitys.ID AND facilitys.lab='$labss'	
			ORDER BY samples.datereceived ASC";
		
			$a = mysql_query($qury) or die(mysql_error());
			$noofsamples=mysql_num_rows($a);

$reason= "SAMPLES RECEIVED ON ". strtoupper($displaydatefilter);
...
$batchno="LOG BOOK FOR SAMPLES RECEIVED ON ".$displaydatefilter.".pdf";
$F="I";
