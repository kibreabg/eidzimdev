...
define('FPDF_FONTPATH', 'font/');
	...
	if ($quarterly == 1 ) //january - March
		{
			$qury = "SELECT * FROM samples s, facilitys f WHERE MONTH(s.datereceived) BETWEEN 1 AND 3 AND YEAR(s.datereceived) = '$quarteryear' AND s.flag = 1 AND s.flag = 1 AND s.facility = f.ID AND f.lab = '$labss' AND s.repeatt=0 ORDER BY s.datereceived DESC ";
$a = mysql_query($qury) or die(mysql_error());
$noofsamples=mysql_num_rows($a);
		}
		else if ($quarterly == 2) //april - june
		{
		$qury = "SELECT * FROM samples s, facilitys f WHERE MONTH(s.datereceived) BETWEEN 4 AND 6 AND YEAR(s.datereceived) = '$quarteryear' AND s.flag = 1 AND s.facility = f.ID AND f.lab = '$labss' AND s.repeatt=0 ORDER BY s.datereceived DESC ";
$a = mysql_query($qury) or die(mysql_error());
$noofsamples=mysql_num_rows($a);
		}
		else if ($quarterly == 3) //july - september
		{
		$qury = "SELECT * FROM samples s, facilitys f WHERE MONTH(s.datereceived) BETWEEN 7 AND 9 AND YEAR(s.datereceived) = '$quarteryear' AND s.flag = 1 AND s.facility = f.ID AND f.lab = '$labss' AND s.repeatt=0 ORDER BY s.datereceived DESC ";
$a = mysql_query($qury) or die(mysql_error());
$noofsamples=mysql_num_rows($a);
		}
		else if ($quarterly == 4) //october - december
		{
		$qury = "SELECT * FROM samples s, facilitys f WHERE MONTH(s.datereceived) BETWEEN 10 AND 12 AND YEAR(s.datereceived) = '$quarteryear' AND s.flag = 1 AND s.facility = f.ID AND f.lab = '$labss' AND s.repeatt=0 ORDER BY s.datereceived DESC";
$a = mysql_query($qury) or die(mysql_error());
$noofsamples=mysql_num_rows($a);
		}
...
$this->SetFont('Arial','B',10);			// Arial bold 15
 ...
 //	$this->SetTextColor(204,204,204);
	...
	$this->Cell(217,8," INFANT DIAGNOSIS  REPORT FOR SAMPLES RECEIVED IN QUARTER ".sprintf($quotaname) . " , ".sprintf($quarteryear)  ,0,0,'C');	// Title 
...
$this->Ln(3);
$this->Cell($offset7);
$this->Cell(160, 3, 'New Laboratory Complex 2nd Floor' , 0, 0, 'L', 0);//KEMRI HQ, Mbagathi Road, Nairobi
$this->Ln(3);
	$this->Cell($offset7);
$this->Cell(160, 3, 'Harare Central Hospita, P.O. Box ST 749,Southerton ,Harare, Zimbabwe' , 0, 0, 'L', 0);//020 2722541 Ext: 2256/2290  / 0725 793260 / 0725 796842
$this->Ln(3);
$this->Cell($offset7);
...
//Create file
$display="Filtered Report of samples Received for quarter ". $quotaname ." , ".$quarteryear .".pdf";
$F="I";
