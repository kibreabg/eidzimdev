...
//get is mother in art
function GetmotheronART($mid)
{
	$getmother = "SELECT mothers.onart as 'MotherOnArt' FROM mothers WHERE mothers.ID='$mid'";
	$gotmother = mysql_query($getmother) or die(mysql_error());
	$motherrec = mysql_fetch_array($gotmother);
	$monart = $motherrec['MotherOnArt'];
	
	return $monart;

}
//paient prophylaxis
function GetProphylaxisName($prophylaxis)
{
	$result = "SELECT name FROM prophylaxis WHERE ID = '$prophylaxis'";
	$getresult = mysql_query($result) or die(mysql_error());
	$resulttype = mysql_fetch_array($getresult);
	$showprof = $resulttype['name'];
