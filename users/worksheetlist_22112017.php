...
<div  class="section">
    ...
    <div class="xtop">
        <?php
        if (isset($wtype)) {
            include("http://localhost:8099/eid1/users/worksheetslink.php?wtype=$wtype");
        } else {
            include("http://localhost:8099/eid1/users/worksheetslink.php");
        }
...
// print the districts info in table
                    ...
                    <tr ><th><small>Serial #</small></th><th><small>Worksheet No</small></th><th><small>Date Created</small></th><th><small>Created By</small></th><th><small>Type</small></th><th><small>No. of Samples</th><th><small>Lot No</small></th><th><small>Date Run</small></th><th><small>Date Reviewed</small></th><th><small>Status</small></th><th><small>Task</small></th></tr>';
...
// print the link to access each page
                    ...
                    for ($page = 1; $page <= $maxPage; $page++) {
                        if ($page == $pageNum) {
                            ...
                            $nav .= " <a href=\"$self?page=$page&wtype={$_GET['wtype']}\">$page</a> ";
                        }
                    }
...
// the first and last page
                    ...
                    if ($pageNum > 1) {
                        $page = $pageNum - 1;
                        $prev = " <a href=\"$self?page=$page&wtype={$_GET['wtype']}\">[Prev]</a> ";

                        $first = " <a href=\"$self?page=1&wtype={$_GET['wtype']}\">[First Page]</a> ";
                    } else {
                    ...
                    if ($pageNum < $maxPage) {
                        $page = $pageNum + 1;
                        $next = " <a href=\"$self?page=$page&wtype={$_GET['wtype']}\">[Next]</a> ";

                        $last = " <a href=\"$self?page=$maxPage&wtype={$_GET['wtype']}\">[Last Page]</a> ";
                    } else {
...
// print the navigation link
            ...
            } elseif ($wtype == 0) { //pending worksheets
                $rowsPerPage = 15; //number of rows to be displayed per page
                // by default we show first page
                $pageNum = 1;
...
// print the navigation link
            ...
            elseif ($wtype == 2) { //manual worksheets
                $rowsPerPage = 15; //number of rows to be displayed per page
// by default we show first page
                $pageNum = 1;
...
// print the navigation link
            ...
            } elseif ($wtype == 4) {//worksheets awating first approval
                $rowsPerPage = 15; //number of rows to be displayed per page
                // by default we show first page
                $pageNum = 1;
...
// print the navigation link
            ...
            } elseif ($wtype == 5) { //worksheets awaiting second approval
                $rowsPerPage = 15; //number of rows to be displayed per page
                // by default we show first page
                $pageNum = 1;
...
<tr><th><small>Serial #</small></th><th><small>Worksheet No</small></th><th><small>Date Created</small></th><th><small>Created By</small></th><th><small>Type</small></th><th><small>No. of Samples</th><th><small>Lot No</small></th><th><small>Date Run</small></th><th><small>Date Reviewed</small></th><th><small>Status</small></th><th><small>Task</small></th></tr>';
                while (list($ID, $worksheetno, $datecreated, $HIQCAPNo, $spekkitno, $createdby, $Lotno, $Rackno, $Flag, $daterun, $datereviewed, $type) = mysql_fetch_array($result)) {
                    ...
                    if ($Flag == 0) {
                        $status = " <strong><small><font color='#0000FF'> In Process  </small></font></strong>";
                        if ($type == 0) {
                            $d = "<a href=\"worksheetDetails.php" . "?ID=$worksheetno&view=1" . "\" title='Click to view Samples in this Worksheet'>View Details</a> | <a href=\"downloadworksheet.php" . "?ID=$worksheetno" . "\" title='Click to Download Worksheet' target='_blank'>Print Worksheet </a> | <a href=\"deleteworksheet.php" . "?ID=$worksheetno&serial=$ID" . "\" title='Click to Delete Worksheet' OnClick=\"return confirm('Are you sure you want to delete Worksheet $worksheetno');\" >Delete Worksheet  </a>";
                            ...
                            if ($wSheet['approvalStatus'] == 1) {
                                if ($userID != $wSheet['firstApprovalBy']) {
                                    ...
                                    $dl = "";
                            } elseif ($wSheet['approvalStatus'] == 0) {
                                $dl = "| <a href=\"confirmresults.php?q={$worksheetno}" . "\" title='Click to Update Results Worksheet' > Approve Results </a>";
                            } else {
                                $dl = "| <a href=\"updateresults.php" . "?ID=$worksheetno" . "\" title='Click to Update Results Worksheet' > Update Results </a>";
                            }
