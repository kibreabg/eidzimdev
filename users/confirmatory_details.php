...
function select(a) {
    ...
    for (i=0; i<theForm.elements.length; i++) {
		...
		<div  class="section">
		<div class="section-title">CONFIRMATORY TEST HISTORY FOR SAMPLE<strong> <?php echo $samplecode; ?></strong></div>
   ...
   <?php
		...
		//query database for all districts
		   $qury = "SELECT samples.ID,samples.patient,samples.datereceived,samples.spots,samples.datecollected,samples.receivedstatus,samples.facility,samples.datetested
					...
					WHERE samples.patient='$samplecode' AND samples.facility=facilitys.ID AND facilitys.lab='$labss'
