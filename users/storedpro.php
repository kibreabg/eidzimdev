...
BEGIN
IF month = 0 THEN                  
      SELECT COUNT(samples.ID) INTO numsamples FROM samples,patients WHERE samples.patient =patients.ID AND patients.age < 2 AND samples.result > 0 AND YEAR(samples.datetested)=yea  AND ((samples.parentid=0) OR (samples.parentid IS NULL)) AND samples.Flag=1;            
  ELSE

 SELECT COUNT(samples.ID) INTO numsamples FROM samples,patients WHERE samples.patient =patients.ID AND patients.age < 2 AND samples.result > 0 AND YEAR(samples.datetested)=yea  AND MONTH(samples.datetested)=month AND ((samples.parentid=0) OR (samples.parentid IS NULL)) AND samples.Flag=1;          
  END IF; 
...
IF month =0 THEN                  
  SELECT COUNT(samples.ID) INTO numtests
            FROM samples,patients,mothers WHERE   samples.result =resulttype AND samples.patient=patients.ID  AND YEAR(samples.datetested)=yea  AND patients.mother=mothers.ID AND mothers.prophylaxis=drug AND samples.Flag=1 ;       
  ELSE
      SELECT COUNT(samples.ID) INTO numtests
            FROM samples,patients,mothers WHERE   samples.result =resulttype AND samples.patient=patients.ID  AND MONTH(samples.datetested)=month   AND YEAR(samples.datetested)=yea  AND patients.mother=mothers.ID AND mothers.prophylaxis=drug AND samples.Flag=1 ;            
  END IF; 
...
BEGIN
IF month = 0 THEN                  
      SELECT COUNT(samples.ID) INTO numsamples
