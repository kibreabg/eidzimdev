...
$qury = "SELECT * FROM samples s, facilitys f WHERE YEAR(s.datereceived) = '$year' AND s.flag = 1 AND s.facility = f.ID AND f.lab = '$labss' AND s.repeatt=0 ORDER BY s.datereceived DESC ";
...
$this->SetFont('Arial','B',10);			// Arial bold 15
 ...
 //	$this->SetTextColor(204,204,204);
	...
	$this->Cell(217,8," INFANT DIAGNOSIS  REPORT FOR SAMPLES RECEIVED IN ".sprintf($year)  ,0,0,'C');	// Title 
...
$this->Cell(160,3,' NOTE: Failed samples require new sample collection',0,0, 'L');
 $this->Ln(3);
 $this->Cell($offset7);
$this->Cell(160,3,' If you have questions or problems regarding samples, please contact the NMRL Zimbabwe at',0,0, 'L');
$this->Ln(3);
	$this->Cell($offset7);
...
//Create file
$display="Filtered Report of Samples Received in ". $year .".pdf";
$F="I";
