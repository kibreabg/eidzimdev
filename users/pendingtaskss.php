...
<div  class="section">
<div class="section-title">SAMPLES WAITING TESTING [ <?php echo samplesawaitingtests(); ?> ]</div>
<div class="xtop">
<table><tr><td>
<A HREF='javascript:history.back(-1)'><img src='../img/back.gif' alt='Go Back'/></A> 
<?php if ((samplesawaitingtests()) > 0) {
?>| <small><strong><font color="#FF0000">The samples have been ordered by Date Received Aecending (Oldest - Earliest)</font></strong></small><?php }?>
</td></tr></table>
...
FROM   samples
WHERE ID='$sample' and batchno='$batchno' AND  ((result IS NULL) OR (result <1))  order by datereceived asc LIMIT " . ($pageNum-1)*$Limit . ",$Limit")or die(mysql_error());

   $samplescount=mysql_query("SELECT ID
FROM   samples
WHERE ID='$sample' and batchno='$batchno' AND  ((result IS NULL) OR (result <1)) ")or die(mysql_error());
