...
if ($dispatch != "" && $checkbox != "") {
    ...
    foreach ($checkbox as $a => $b) { //update worksheet items with date dispac v
        ...
        $mother = GetMotherANC($patientid);
        $motherAnc = $mother['anc']; //Added
        $faclty = getFacilityDetails($facility);
        $facilityCode = $faclty['facilitycode']; //Added
        $facilityName = $faclty['name'];
        $telNo = $faclty['telephone']; //Added
        $imei = $faclty['imei'];
        $pass = $faclty['pass'];
        $distID = GetDistrictID($facility);
        $district = GetDistInfo($distID);
        ...
        $province = GetProvInfo($provID);
        $provName = $province['name'];

        $pt = GetPatientInfo($patientid);
        $patientName = $pt['name'];
        $ptName = preg_split('/\s+/', $patientName, -1, PREG_SPLIT_NO_EMPTY);
        $infantFirstName = $ptName[0];
        $infantLastName = $ptName[1];
        $dob = $pt['dob'];
        $labReqNo = $pt['ID'];
        ...
        $queryPrintQ = "INSERT IGNORE
                        INTO smsprintqueue (sampleid, datequeued, status, failedprinting, successfullyprinted, facilityname, facilityid, facilityCode, telephoneNo, dbsrequestno, nmrlStampNo, patientname, motherANC, dateofbirth, dateReceived, datecollected, dateReleased, datetested, receivedStatus, result, rejectedReason, lastupdatedate, imei, pass, provinceid, provinceName, districtid, districtName)
                        VALUES ('{$labcode[$a]}','{$dateQueued}','" . Queued . "','0','0','{$facilityName}','{$facility}','{$facilityCode}','{$telNo}','{$patient}','{$nmrlstampno}','{$patientName}','{$motherAnc}','{$dob}','{$datereceived}','{$datecollected}','{$datereleased}','{$datetested}','{$receivedstatus}','{$rslt}','{$rejectedreason}','{$lastUpdateDate}','{$imei}','{$pass}','{$provID}','{$provName}','{$distID}','{$distName}')";

        $queryPrintQSQL = "INSERT INTO smsprintqueue
           (guid
           ,sampleID
           ,dateQueued
           ,status
     ...
     VALUES
        ...
        $params = array(getGUID(), $labcode[$a], $dateQueued, Queued, 0, 0, $facilityName, $facility, $facilityCode, $telNo, $patient, $nmrlstampno, $patientName, $motherAnc, $dob, $datereceived, $datecollected, $datereleased, $datetested, $receivedstatus, $rslt, $rejectedreason, $lastUpdateDate, $imei, $pass, $provID, $provName, $distID, $distName, 'NMRL');

        $smsQueue = mysql_query($queryPrintQ) or die(mysql_error());
        $smsQueueSQL = sqlsrv_query($mssql_link, $queryPrintQSQL, $params);

        //CURL Requests
        // Get cURL resource
        $curl = curl_init();
        $infantSearch = 'http://192.168.0.24:8080/api/trackedEntityInstances.json?filter=HJVik7nYqdZ:EQ:'. $infantFirstName .'&filter=QTH0s2ybnN1:EQ:'. $infantLastName .'&ou=qKmB3CBc0NN';
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_URL => $infantSearch,
            CURLOPT_USERPWD => "gweru" . ":" . "Tracker123*",
            CURLOPT_HTTPHEADER => 'Content-Type: application/json',

        ));
        // Find the Tracked Entity Instance (The Infant)
        $resp = curl_exec($curl);
        $obj = json_decode($resp, true);
        $trackedEntity = $obj['trackedEntityInstances'][0]['trackedEntityInstance'];
        // Find the event on that tracked entity instance with the specified Lab Request No.
        $eventSearch = 'http://192.168.0.24:8080/api/29/events.json?filter=GZ99KQmDtIX:EQ:'. $labReqNo .'&orgUnit=qKmB3CBc0NN&trackedEntityInstance=' . $trackedEntity;
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_URL => $eventSearch,
            CURLOPT_USERPWD => "gweru" . ":" . "Tracker123*",
            CURLOPT_HTTPHEADER => 'Content-Type: application/json',
        ...
        $resp = curl_exec($curl);
        $updatedEvent = json_decode($resp, true);
        unset($updatedEvent["href"]);
        unset($updatedEvent["event"]);
        $index = 0;
        $dataValueIndex = 0;
        /*foreach ($updatedEvent['dataValues'] as $item) {
            if ($item['dataElement'] == "fhV2pWcHLSJ") {
                $item['value'] = $rslt;
                $dataValueIndex = $index;
                $updatedEvent['dataValues'][$index] = $item;
            }
            ++$index;
        }*/
        $hivResultJson = array();
        $hivResultJson['dataElement'] = 'fhV2pWcHLSJ';
        $hivResultJson['value'] = $rslt;      
        $updatedEvent['dataValues'][] = $hivResultJson;
        //Add Date Result Posted to DHIS2
        $now = gmdate('Y-m-d\TH:i:s');
        $datePostedJson = array();
        $datePostedJson['dataElement'] = 'cuwkpTKgrL9';
        $datePostedJson['value'] = $now;      
        $updatedEvent['dataValues'][] = $datePostedJson;

        //$eventApiUrl = 'http://192.168.0.24:8080/api/29/events';

        $updatedEvent = json_encode($updatedEvent);
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_URL => $eventUrl,
            CURLOPT_USERPWD => "gweru" . ":" . "Tracker123*",
            CURLOPT_HTTPHEADER => array('Content-Type: application/json', 'Content-Length: ' . strlen($updatedEvent)),
            CURLOPT_POSTFIELDS => $updatedEvent,
    ...
    $sampleIds = $_SESSION['sampleIDs'];

    foreach ($checks as $a => $b) {

    $dateQueued = date("Y-m-d h:i:s");
    $sample = getSampleetails($sampleIds[$b]);
    extract($sample);
    ...
    VALUES ('{$sampleIds[$b]}','{$dateQueued}','" . Queued . "','0','0','{$facilityName}','{$facility}','{$patient}','{$patientName}', '{$dob}', '{$datecollected}', '{$datetested}', '{$rslt}', '{$lastUpdateDate}','{$imei}','{$pass}','{$provID}','{$distID}')";
    ...
    // Set parameter values.
    $params = array(getGUID(), $sampleIds[$b], $dateQueued, Queued, 0, 0, $facilityName, $facility, $patient, $patientName, $dob, $datecollected, $datetested, $rslt, $lastUpdateDate, $imei, $pass, $provID, $distID, 'NMRL');
