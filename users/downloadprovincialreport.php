...
$qury = "SELECT * 
FROM samples s, facilitys f,districts d 
WHERE s.facility = f.ID AND f.district = d.id AND d.province = $province AND s.datereceived BETWEEN '$startdate' AND '$enddate' AND s.flag = 1 AND f.lab = '$labss' AND s.repeatt=0 ORDER BY s.datereceived ASC ";
...
$this->SetFont('Arial','B',10);			// Arial bold 15
 ...
 //	$this->SetTextColor(204,204,204);
	...
	$this->Cell(217,8," HIV DNA PCR LABORATORY REQUEST FORMS RECEIVED BETWEEN ".sprintf($dislaystartdate) . " & ".sprintf($dislayenddate) . " FOR " .sprintf($provincename)." PROVINCE" ,0,0,'C');	// Title 
...
$this->Cell($offset);
 ...
 $this->SetFillColor(204,204,204);	
 	//$this->SetTextColor(204,200,214);
		...
		$header=array('Sample Request No ','Age (Mths)','Gender','Date Collected','Date Received','Received Status','Date of Test','Test Result','Date of Dispatch','TAT(dys)');
		$w=array(40,20,20,30,30,25,30,25,30,15);
