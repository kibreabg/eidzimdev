...
$searchparameter = ltrim(rtrim($_POST['nmrlno'])); //get the search parameter from the userheader and trim the value
$searchparameterid = ltrim(rtrim($_POST['nmrlstampno'])); //get the search parameter from the userheader and trim the value
...
<div>
	...
	?>
		...
		<?php		
			...
			$showsample = "SELECT ID,LTRIM(RTRIM(patient)),facility,nmrlstampno,batchno,receivedstatus,rejectedreason,spots,datecollected,datereceived,datetested,datemodified,datedispatched,result,worksheet,sampleokforretest,repeatt,approved FROM samples WHERE nmrlstampno = '$searchparameter' AND flag = 1";
			...
			if ($showcount!=0) //display table is count is NOT 0
			{
				...
				<table   >
			<tr>
			<td style="width:auto" ><div class="notice"><?php echo "The search for Sample with NMRL  # <strong>".LTRIM(RTRIM($searchparameter))."</strong> returned <strong>".$showcount."</strong> results.<br/>"; ?></div></th>
			</tr>
			</table>
			<?php
				echo '<table border="0" >';	
...
<A HREF='javascript:history.back(-1)'><img src='../img/back.gif' alt='Go Back'/></A>
		...
		<tr ><th><small>Count</small></th><th><small>Lab # </small></th><th> <small>Nmrl stamp No</small></th><th><small>Request # </small></th><th><small>Facility</small></th>  <th><small>Batch No</small></th><th><small>Received Status</small></th><th><small>Date Collected</small></th><th><small>Date Received</small></th><th><small>Date Tested</small></th><th><small>Date Modified</small></th><th><small>Date Dispatched</small></th><th><small>Status</small></th><th><small>Result</small></th><th><small>Worksheet</small></th><th><small>Task</small></th></tr>";
				
				
					while(list($ID,$patient,$facility,$nmrlstampno,$batchno,$receivedstatus,$rejectedreason,$spots,$datecollected,$datereceived,$datetested,$datemodified,$datedispatched,$result,$worksheet,$sampleokforretest,$repeatt,$approved) = mysql_fetch_array($displaysample))
				{  
