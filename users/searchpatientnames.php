...
$searchparameter = ltrim(rtrim($_POST['patientname'])); //get the search parameter from the userheader and trim the value
$searchparameterid = ltrim(rtrim($_POST['patientid'])); //get the search parameter from the userheader and trim the value
...
<div>
	...
	?>
		...
		<?php		
			...
			$showsample = "SELECT patients.name,samples.ID,LTRIM(RTRIM(samples.patient)),samples.facility,samples.batchno,samples.receivedstatus,samples.rejectedreason,samples.spots,samples.datecollected,samples.datereceived,samples.datetested,samples.datemodified,samples.datedispatched,samples.result,samples.worksheet,samples.sampleokforretest,samples.repeatt,samples.approved FROM patients,samples WHERE samples.patientid =patients.AutoID and patients.name = '$searchparameter' AND samples.lab='$labss' AND samples.flag=1";
			...
			if ($showcount!=0) //display table is count is NOT 0
			{
				...
				<table   >
			<tr>
			<td style="width:auto" ><div class="notice"><?php echo "The search for Sample with Patient Name(s) <strong>".LTRIM(RTRIM($searchparameter))."</strong> returned ".$showcount." results.<br/>"; ?></div></th>
			</tr>
			</table>
			<?php
				echo '<table border="0" >';	
...
<A HREF='javascript:history.back(-1)'><img src='../img/back.gif' alt='Go Back'/></A>
		...
		<tr ><th><small>Count</small></th><th><small>Lab # </small></th><th><small>Request # </small></th><th><small>Patient Names </small></th><th><small>Facility</small></th><th><small>Batch No</small></th><th><small>Received Status</small></th><th><small>Date Collected</small></th><th><small>Date Received</small></th><th><small>Date Tested</small></th><th><small>Date Modified</small></th><th><small>Date Dispatched</small></th><th><small>Status</small></th><th><small>Result</small></th><th><small>Worksheet</small></th><th><small>Task</small></th></tr>";
				
				
					while(list($name,$ID,$patient,$facility,$batchno,$receivedstatus,$rejectedreason,$spots,$datecollected,$datereceived,$datetested,$datemodified,$datedispatched,$result,$worksheet,$sampleokforretest,$repeatt,$approved) = mysql_fetch_array($displaysample))
				{  
