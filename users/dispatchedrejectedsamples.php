...
if ($checkbox != "") {
    ...
    foreach ($checkbox as $a => $b) { //update pending tasks
...
if (isset($_POST['formPrintSubmit'])) {
    ...
    if (empty($selectedList)) {
        ...
        echo "window.location.href='Multi_samplerejected_detailsprint.php?view=1&SelctedL=" . urlencode(serialize($selectedList)) . "'";
        echo '</script>';
    }
}
?>
...
//query database for all districts
        ...
        <form name="fm_dispatch" action="<?php //echo htmlentities($_SERVER['PHP_SELF']);            ?>" method="post">
            <div style="padding-bottom:3px;" align="right">
                <input  class="button1" name="formPrintSubmit" type="submit" value="Print Selected"/>
            </div>
            <?php
            if ($no != 0) {
                ?>
