...
function GetSavedSamples($lastpatientid, $labss, $BatchNo, $pid, $fcode, $srecstatus, $sspot, $sdoc, $datedispatchedd, $sdrec, $scomments, $labcomment, $parentid, $rejectedreason, $repeatreason, $testreason, $othertest, $dateenteredindb, $loggedinby, $nmrlstampno, $rej,$recievedby) {
    if ($parentid == "") {
        $parentid = 0;
    }
    $blank = "";
    $child = "INSERT INTO samples
(patientid,lab,batchno, patient, facility, receivedstatus, spots, datecollected, datedispatchedfromfacility, datereceived, comments, labcomment,parentid,rejectedreason,reason_for_repeat,datetested,worksheet,result,datemodified,datedispatched,fcode,Inworksheet,BatchComplete,DispatchComments,Flag,repeatt,sampleokforretest,test_reason,othertest,dateenteredindb,loggedinby,nmrlstampno,approved,ReceivedBy)
VALUES('$lastpatientid','$labss','$BatchNo', '$pid', '$fcode', '$srecstatus', '$sspot', '$sdoc', '$datedispatchedd', '$sdrec', '$scomments', '$labcomment', '$parentid', '$rejectedreason', '$repeatreason', '$blank', '$blank', '$blank', '$blank', '$blank', '$blank', '$blank','$blank','$blank',1,'$blank','$blank','$testreason','$othertest','$dateenteredindb','$loggedinby','$nmrlstampno','$rej','$receivedby')"; //
    $success = mysql_query($child) or die(mysql_error());
    return $success;
}

//save sampels details
function GetZSavedSamples($lastpatientid, $labss, $BatchNo, $pid, $fcode, $srecstatus, $sspot, $sdoc, $datedispatchedd, $sdrec, $scomments, $labcomment, $parentid, $rejectedreason, $repeatreason, $testreason, $othertest, $infantresult, $inputcomplete, $approved, $dateenteredindb, $loggedinby) {
    if ($parentid == "") {
        $parentid = 0;
    }
    $blank = "";
    $child = "INSERT INTO samples(patientid,lab,batchno, patient, facility, receivedstatus, spots, datecollected, datedispatchedfromfacility, datereceived, comments, labcomment,parentid,rejectedreason,reason_for_repeat,result,Flag,test_reason,othertest,inputcomplete,approved,BatchComplete,sampleokforretest,dateenteredindb,loggedinby)
VALUES('$lastpatientid','$labss','$BatchNo', '$pid', '$fcode',1, '$sspot', '$sdoc', '$datedispatchedd', '$sdrec', '$scomments', '$labcomment', '$parentid', '$rejectedreason', '$repeatreason', '$infantresult',1,'$testreason','$othertest',1,1,1,0,'$dateenteredindb','$loggedinby')";
    $success = mysql_query($child) or die(mysql_error());
    return $success;
}
...
//save user details
function SaveUser($surname, $oname, $telephone, $postal, $email, $account, $username, $password, $lab, $datecreated,$test) {

    $saved = "INSERT INTO 		
users(surname,oname,telephone,postal,email,account,lab,username,password,flag,datecreated,test)VALUES('$surname','$oname','$telephone','$postal','$email','$account','$lab','$username','$password',1,'$datecreated','$test')";
...
function weeklyrejectedsamples($lab, $startdate, $enddate) {
    $samplequery = "SELECT COUNT(samples.ID) as num_samples FROM samples,facilitys WHERE samples.facility=facilitys.ID   AND facilitys.lab='$lab' AND samples. datetested BETWEEN '$startdate' AND '$enddate' AND samples.receivedstatus=2";
    $sampleresult = mysql_query($samplequery) or die(mysql_error());
    $samplerow = mysql_fetch_array($sampleresult, MYSQL_ASSOC);
    $num_samples = $samplerow['num_samples'];
...
function monthlyrejectedsamples($lab, $month, $year) {
    $samplequery = "SELECT COUNT(samples.ID) as num_samples FROM samples,facilitys WHERE samples.facility=facilitys.ID   AND facilitys.lab='$lab' AND MONTH(samples.datetested)=  '$month' AND YEAR(samples.datetested)= '$year'  AND samples.receivedstatus=2";
    $sampleresult = mysql_query($samplequery) or die(mysql_error());
    $samplerow = mysql_fetch_array($sampleresult, MYSQL_ASSOC);
    $num_samples = $samplerow['num_samples'];
