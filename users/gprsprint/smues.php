...
function Get_SampleInfo($pid) {
    $samplesquery = mysql_query("SELECT datecollected,datetested,facility FROM samples where patient=" . $pid . " order by datetested desc limit 1 ") or die(mysql_error());
...
if (isset($_GET['facility']) && isset($_GET['pid']) && isset($_GET['res'])) {
    ...
    if ($response == false) {
        ...
        echo "window.location.href='../smsprintqueue.php?smsstatus=-1&view=1'";
        echo '</script>';
    } else {
        //echo $print_message;	
...
function ResponseResult($response, $facilityname, $pid) {
    ...
    if ($response['status'] == 'fail') {
        ...
        if (isset($_GET['facility']) && isset($_GET['pid']) && isset($_GET['res'])) {
            ...
            echo '<script type="text/javascript">';
            echo "window.location.href='../smsprintqueue.php?smsstatus=" . $smsstatus . "&view=1&sfacility=" . $facility . "&spid=" . $pid . "&errorCode=" . $error['code'] . "&errormsg=" . $error['message'] . "'";
            echo '</script>';
        } else {
            UpdateSentStatus($pid, 0);
