...
//echo $requestUrl;
	...
	if($response==false)
		...
		echo "window.location.href='../gprsprint.php?smsstatus=-1&view=1'";
				echo '</script>';
					}
					else
					{
		  ...
		  if(isset($_GET['facility'])&& isset($_GET['pid']) && isset($_GET['res']))
			{
			  ...
			  //..redirect to the dispatchedResults page
				echo '<script type="text/javascript">' ;
				echo "window.location.href='../gprsprint.php?smsstatus=".$smsstatus."&view=1&sfacility=".$facility."&spid=".$pid."&errorCode=".$error['code']."&errormsg=".$error['message']."'";
				echo '</script>';
			}
		  else
		  {
