...
header("Content-Disposition: attachment;Filename=Quartely_Excel_Report.xls");
//readfile('Quartely_Excel_Report.xls');
?>
<?php
include("nationaldashboardfunctions.php");
	...
	$quotaname= GetQuarterName($quarterly);
	if ($quarterly == 1 ) //january - March
		...
		$endmonth=12;
					...
					if (($province==0) &&  ($facility==0) ) //all
					{
						...
						$title= $quarterly . " [ ".$quotaname . " ] ". " ," .$quarteryear ;
						$showfacility = "SELECT samples.ID,samples.patientid,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples WHERE facilitys.ID=samples.facility  AND YEAR(samples.datetested)='$quarteryear' AND MONTH(samples.datetested) BETWEEN '$startmonth' AND '$endmonth' AND samples.repeatt=0 AND  samples.flag=1 order by facilitys.name ASC";
					...
					elseif (($province !=0) &&  ($facility==0) ) //provincial
					{
						...
						$title= $provname . " Province , QUARTER ". $quarterly . " [ ".$quotaname . " ] ". " ," .$quarteryear ;
						$showfacility = "SELECT samples.ID,samples.patientid,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples,districts WHERE facilitys.ID=samples.facility AND facilitys.district=districts.ID    AND districts.province='$province' AND YEAR(samples.datetested)='$quarteryear' AND MONTH(samples.datetested) BETWEEN '$startmonth' AND '$endmonth' AND samples.repeatt=0 AND  samples.flag=1 order by facilitys.district ASC";
			
					}
					/*else if ($radio=="radio3") //district
					{
						$distname=GetDistrictName($district);
						$title= $distname . " District , QUARTER ". $quarterly . " [ ".$quotaname . " ] ". " ," .$quarteryear ;
						$showfacility = "SELECT samples.ID,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples WHERE facilitys.ID=samples.facility AND facilitys.district='$district' AND YEAR(samples.datetested)='$quarteryear' AND MONTH(samples.datetested) BETWEEN '$startmonth' AND '$endmonth' AND samples.repeatt=0 AND  samples.flag=1 order by facilitys.name ASC";
					...
					elseif (($province ==0) &&  ($facility !=0) )//facility
					{
						...
						$title= $facilityname . " , QUARTER ".$quarterly . " [ ".$quotaname . " ] ". " ," .$quarteryear ;
						
						$showfacility = "SELECT samples.ID,samples.patientid,samples.patient,samples.facility,samples.datecollected,samples.datereceived,samples.receivedstatus,samples.spots,samples.datetested,samples.result,samples.datemodified,samples.datedispatched,samples.lab as 'labtestedin' FROM facilitys,samples WHERE facilitys.ID=samples.facility AND samples.facility='$facility' AND  YEAR(samples.datetested)='$quarteryear' AND MONTH(samples.datetested) BETWEEN '$startmonth' AND '$endmonth' AND samples.repeatt=0 AND  samples.flag=1 ";
